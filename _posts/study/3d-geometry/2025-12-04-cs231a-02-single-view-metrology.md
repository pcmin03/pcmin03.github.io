...
이전 강의 노트에서는 카메라의 외부 및 내부 속성을 사용하여 실제 3D 세계의 점들을 디지털 이미지로 변환하는 방법을 논의했습니다. 보정 장치의 알려진 구조와 해당 이미지를 사용하여 이러한 카메라 속성을 추론하는 방법을 살펴보았습니다. 이번에는 관련된 문제를 살펴보겠습니다: 단일 이미지가 있고 그 이미지를 촬영한 카메라의 속성을 알고 있다면, 3D 세계의 알려진 구조를 복원할 수 있을까요? 그런 다음 단일 이미지로부터 추론할 수 있는 정보에 대해 더 일반적으로 논의하겠습니다.

## 2. Transformations in 2D

이미지로부터 학습하는 방법을 더 잘 이해하기 위해, 먼저 2D 공간의 다양한 변환에 대해 알아야 합니다.
...
여기서 $\begin{bmatrix} x' & y' & 1 \end{bmatrix}^T$는 등거리 변환 후에 달성된 점입니다.

**Similarity transformations(유사 변환)**은 모양을 보존하는 변환입니다. 직관적으로, 등거리 변환이 할 수 있는 모든 것에 스케일링을 더한 것입니다. 수학적으로 다음과 같이 표시됩니다:
...
모양을 보존하기 때문에 길이와 각도의 비율도 보존합니다. 모든 등거리 변환은 $s = 1$일 때 유사 변환의 특정 형태임을 주목하세요. 그러나 그 역은 성립하지 않습니다.
...
평행성을 보존하지 않지만, 투영 변환은 직선을 직선으로 매핑하기 때문에 점의 공선성을 보존합니다. 또한 네 개의 공선 점의 **cross ratio(교차비)**가 투영 변환 하에서 불변임을 증명합니다. 교차비는 직선 위의 네 점 $P_1, P_2, P_3, P_4$를 취하여 다음을 계산합니다:

$$\text{cross ratio} = \frac{\|P_3 - P_1\| \|P_4 - P_2\|}{\|P_3 - P_2\| \|P_4 - P_1\|} \tag{1}$$

교차비의 투영 변환 하에서의 불변성 증명은 수업 연습으로 남겨둡니다.
...
평행선의 경우는 어떨까요? 일상적인 지식은 이러한 직선이 절대 교차하지 않을 것으로 예상합니다. 그러나 이 정의는 이러한 직선이 무한대에서 교차한다고 다시 쓸 수 있습니다. 동차 좌표에서 무한대의 점은 $\begin{bmatrix} x & y & 0 \end{bmatrix}^T$로 작성됩니다. 유클리드 좌표는 마지막 좌표로 모든 좌표를 나누어 수집됩니다. 이 경우 좌표가 0이므로 무한대의 점을 달성합니다. 따라서 동차 좌표는 평행선의 경우에도 교차점을 결정하는 좋은 공식을 제공합니다.

이제 두 평행선 $\ell$과 $\ell'$을 고려해보겠습니다. 두 직선이 평행할 때, 기울기가 같으므로 $\frac{a}{b} = \frac{a'}{b'}$입니다. 동차 좌표를 사용하여 교차점을 계산하면 다음을 확인합니다:

$$\ell \times \ell' \propto \begin{bmatrix} b \\ -a \\ 0 \end{bmatrix} = x_\infty \tag{3}$$

따라서 두 평행선이 무한대에서 교차한다는 우리의 직관을 확인했습니다. 두 평행선의 무한대에서의 교차점은 **ideal point(이상점)**이라고도 합니다. 무한대의 점의 흥미로운 속성 중 하나는 동일한 기울기 $-\frac{a}{b}$를 가진 모든 평행선이 아래에 표시된 대로 이상점을 통과한다는 것입니다:

$$\ell^T x_\infty = \begin{bmatrix} a & b & c \end{bmatrix} \begin{bmatrix} b \\ -a \\ 0 \end{bmatrix} = 0 \tag{4}$$

무한대의 점의 개념은 **lines at infinity(무한대의 직선)**를 정의하도록 확장될 수 있습니다. 두 개 이상의 평행선 쌍을 고려하세요. 각 평행선 쌍은 무한대의 점 $\{x_{\infty,1}, \ldots, x_{\infty,n}\}$에서 교차합니다. 이러한 모든 무한대의 점을 통과하는 직선 $\ell_\infty$는 $\forall i, \ell_\infty^T x_{\infty,i} = 0$을 만족해야 합니다. 이것은 $\ell_\infty = \begin{bmatrix} 0 & 0 & c \end{bmatrix}^T$를 의미합니다. $c$는 임의의 값이므로, 단순히 $\ell_\infty = \begin{bmatrix} 0 & 0 & 1 \end{bmatrix}^T$로 정의할 수 있습니다.
...
지금까지 2D에서 무한대의 직선과 점의 개념을 소개했습니다. 이제 해당 동차 좌표에서 3D에 대한 동등한 개념을 소개하겠습니다.
...
3D의 직선은 두 평면의 교차로 정의됩니다. 4개의 자유도(정의된 절편 위치와 세 차원 각각의 기울기)를 가지기 때문에, 3D 공간에서 잘 표현하기 어렵습니다. 자세한 내용은 Hartley & Zisserman 교과서의 섹션 3.2.2를 참조하세요.
...
위 방정식의 유도는 연습으로 남겨둡니다. 이 방정식은 방향 $d$를 추출하도록 다시 쓸 수 있습니다:
...
수평선의 개념은 수학적으로 쉽게 명백하지 않을 수 있는 이미지에 대한 속성을 직관적으로 추론하는 인간의 능력을 허용합니다. 예를 들어, Figure 2에서 지면의 선들이 이미지 좌표에서 평행하지 않지만, 3D 세계에서 평행하다는 자연스러운 이해를 가지고 있습니다.
